...
* **<%= vars.product_short %> CLI:** For information about installing the <%= vars.product_short %> CLI, see
[Install the <%= vars.product_short %> CLI](./installing.html#cli) .

* **Helm 3 CLI:** For information about installing the Helm CLI,
see the [Helm documentation](https://helm.sh/docs/intro/).

* **Docker CLI:** For information about installing Docker, see [Docker](https://www.docker.com/).
...
1. Get the following resources:
    ...
    commands for the **broker**, **daemon**, **chartmuseum**, and **postgresql**.
...
1. Ensure <%= vars.product_short %> CLI is working properly:
...
1. Tag the images for your registry by running these commands:
    ...
    + `CHARTMUSEUM-VERSION-NUMBER` is the ChartMuseum version number.
      This value is in the `docker pull` command that you recorded in
      [Get <%= vars.product_short %> Resources From <%= vars.product_network %>](#get) above.
    + `POSTGRESQL-VERSION-NUMBER` is the PostgreSQL version number.
      This value is in the `docker pull` command that you recorded in
      [Get <%= vars.product_short %> Resources From <%= vars.product_network %>](#get) above.
...
1. Push the images to your registry by running these commands:
    ...
    docker push REGISTRY/container-services-manager/postgresql:POSTGRESQL-VERSION-NUMBER
    ```
...
1.  In the `ksm/values.yaml` file, add the credentials for your S3-compatible bucket using the template below:
    ...
    <p class="note">
      ...
      For example, you might need to set <code>STORAGE_AMAZON_REGION</code>
      if you are not using using the default region.
...
1. Edit your `ksm/values.yaml` file to enable Ingress service access.
...
1. Define a secure password to authenticate your services by adding:
...
1. Define values for your registry by adding:
...
1. Add the credentials for the registry where the <%= vars.product_short %> images are by adding:
...
1. Add your Cloud Foundry environment by adding:
...
1. Specify which Postgres database to use:
    + **Internal Postgres Configuration:** VMware discourages internally
      configuring Postgres for production environments.
    ...
    + **External Postgres configuration:**
        ...
        ```
        Where:
        + `REGISTRY` is the registry location for the <%= vars.app_runtime_abbr %> image.
        + `USERNAME` is the PostgreSQL username.
        + `PASSWORD` is the PostgreSQL password.
        + `DATABASE` is the schema within PostgreSQL where the tables for <%= vars.product_short %> are located.
        + `ENCRYPTION-KEY` is used to symmetrically encrypt sensitive data before you save it in PostgreSQL.
        ...
        the PostgreSQL database connection. This certificate is required when `dbSkipSSL` is `false`.
...
1. From the root level of the chart, install the Helm chart by running these commands:
    ...
    helm install RELEASE-NAME . -n <%= vars.product_short %>-NAMESPACE --wait
    ```
    Where:
...
1. If are using a load balancer:
    ...
    1. Upgrade your Helm release by running:
        ...
        The above commands add your Cloud Foundry environment to <%= vars.product_short %>.
